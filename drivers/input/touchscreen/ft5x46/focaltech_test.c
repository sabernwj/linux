			       ...
			       size_t count, loff_t *pos)
{
	...
	snprintf(tmp, sizeof(focal_test_result), "%d\n", focal_test_result);
...
static ssize_t focal_tp_datadump_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	EnterFactory();
	SysDelay(100);
	cnt = FT5X46_TestItem_GetRawData(buf, 0);
	if (cnt == 0)
		goto out;
	ret = FT5X46_TestItem_GetRawData(buf + cnt, 1);
	if (ret == 0)
		goto out;
out:
	*pos += strlen(buf);
...
static const struct proc_ops tp_selftest_ops = {
	.proc_read		= focal_tp_selftest_read,
	.proc_write		= focal_tp_selftest_write,
};

static const struct proc_ops focal_tp_datadump_ops = {
	.proc_read		= focal_tp_datadump_read,
};

int focal_test_module_init(struct i2c_client *client)
{
	FOCAL_TEST_DBG("[focal] %s ",  FOCALTECH_TEST_INFO);
	focal_selftest_proc = proc_create("tp_selftest", 0, NULL, &tp_selftest_ops);
